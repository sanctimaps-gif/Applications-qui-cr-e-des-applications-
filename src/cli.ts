  ...
  bus.onEvent((event) => {
    switch (event.type) {
      case 'phase':
        ...
        break;
      case 'plan':
        logger.info(`  ${color.dim(`${event.name} · ${event.files} fichiers · ${event.stack}`)}`);
        break;
      case 'file:done':
        if (!quiet) {
          logger.info(
      ...
      case 'command':
        ...
        break;
      case 'repair':
        logger.step(`reparation ${event.attempt}`);
        break;
      case 'github':
...
async function cmdNew(flags: Flags, cfg: ForgeConfig, logger: Logger): Promise<number> {
  ...
  try {
    ...
    if (result.repo) logger.info(`  ${color.dim('depot')}       ${result.repo.url}`);
...
async function main(): Promise<number> {
  ...
  const logger = new Logger(bool(flags, 'quiet', false) ? 'warn' : cfg.logLevel);
